...
def main():
    ...
    for i in range(file_id_start, file_id_stop + 1, 1):
        pcd_file = Path(f"../data/pcd_{i}.pcd")
    ...
    # NOTE: looks like this line applies the correct transform before we ever try to align them - not sure why they're doing that
...
def remove_clouds_outliers(pcds, num_points, radius, ratio=0):
    for i in range(len(pcds)):
        cl, ind = pcds[i].remove_radius_outlier(num_points, radius)
        pcds[i] = pcds[i].select_by_index(ind)

        if ratio > 0:
            cl, ind = pcds[i].remove_statistical_outlier(50, ratio)
            pcds[i] = pcds[i].select_by_index(ind)
